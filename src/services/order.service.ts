...
export class OrdersService{
    ...
    async createNewOrder(userId: number, products: ProductOrderInfo[]): Promise<OrderDetails | null>{
        ...
        await Promise.all(products.map(async (product) => {
            ...
            VALUES($1, $2, $3) RETURNING*`;
             const productsOrders = await conn.query(sql, [product.id, order.id, product.quantity]);
        }));
        return order;
        }
        catch(err){
    ...
    async updateExistingOrder(userId: number, order: OrderDetails): Promise<void | null>{
        try{
            const conn = await client.connect();
